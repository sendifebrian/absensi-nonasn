...
function validasi_tanggal(&$tanggal_mulai, &$tanggal_selesai, &$berlaku_untuk, &$keterangan): ?string {
    ...
    $keterangan      = trim($_POST['keterangan'] ?? 'Work From Home');
...
if ($action === 'tambah') {
    ...
    if ($berlaku_untuk === 'semua') {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO wfh_schedule (tanggal_mulai, tanggal_selesai, keterangan, berlaku_untuk, user_id, created_by)
            VALUES (?, ?, ?, 'semua', NULL, ?)
        ");
        ...
        if (!empty($konflik)) {
            ...
            $_SESSION['alert'] = [
                'type'    => 'warning',
                'message' => "Jadwal WFH untuk semua pegawai berhasil ditambahkan. "
                           . "Perhatian: pegawai berikut sedang cuti/izin/sakit pada rentang ini dan tidak bisa absen WFH — $skip.",
            ];
        } else {
            $_SESSION['alert'] = ['type' => 'success', 'message' => 'Jadwal WFH untuk semua pegawai berhasil ditambahkan.'];
        }
        ...
        if (empty($bisa_ids)) {
            ...
            $_SESSION['alert'] = [
                'type'    => 'danger',
                'message' => "Tidak ada pegawai yang bisa dijadwalkan WFH. Semua sedang cuti/izin/sakit: $info.",
            ];
            header('Location: wfh.php'); exit();
        }
        ...
        if (!empty($konflik)) {
            ...
            $_SESSION['alert'] = [
                'type'    => 'warning',
                'message' => "Jadwal WFH berhasil ditambahkan untuk $jumlah pegawai. "
                           . "Dilewati karena cuti/izin/sakit: $skip.",
            ];
        } else {
            $_SESSION['alert'] = ['type' => 'success', 'message' => "Jadwal WFH berhasil ditambahkan untuk $jumlah pegawai."];
        }
    }
...
if ($action === 'edit') {
    ...
    if ($berlaku_untuk === 'semua') {
        ...
        if (!empty($konflik)) {
            ...
            $_SESSION['alert'] = [
                'type'    => 'warning',
                'message' => "Jadwal WFH diperbarui. Perhatian: pegawai berikut sedang cuti/izin/sakit — $skip.",
            ];
        }
    } else {
        ...
        if (!empty($konflik)) {
            ...
            $_SESSION['alert'] = [
                'type'    => 'warning',
                'message' => "Jadwal WFH disimpan. Perhatian: {$k['nama']} sedang {$k['alasan']} pada rentang tanggal ini.",
            ];
        }
    }
    ...
    if (!isset($_SESSION['alert'])) {
        $_SESSION['alert'] = ['type' => 'success', 'message' => 'Jadwal WFH berhasil diperbarui.'];
    }
    header('Location: wfh.php'); exit();
}
...
if ($action === 'hapus') {
    $id = intval($_POST['id'] ?? 0);
    if ($id > 0) {
        $stmt = $pdo->prepare("DELETE FROM wfh_schedule WHERE id = ?");
        $stmt->execute([$id]);
        $_SESSION['alert'] = ['type' => 'success', 'message' => 'Jadwal WFH berhasil dihapus.'];
    }
    header('Location: wfh.php'); exit();
}
